...
export class TagProtectionChecks {
  ...
  private checkBypass(
    ...
    if (policyBypass.organization_admins) {
      const orgAdminBypass = bypassActors.find(
        (actor: any) => actor.actor_type === "OrganizationAdmin",
      );
      checks.organization_admins = {
        ...
        passed:
          orgAdminBypass?.bypass_mode === policyBypass.organization_admins,
      };
    }

    // Check teams
    if (policyBypass.teams) {
      checks.teams = policyBypass.teams.map((team: any) => {
        const teamBypass = bypassActors.find(
          (actor: any) =>
            actor.actor_type === "Team" && actor.actor_id === team.id,
        );
        return {
          id: team.id,
    ...
    if (policyBypass.integrations) {
      checks.integrations = policyBypass.integrations.map(
        (integration: any) => {
          const integrationBypass = bypassActors.find(
            (actor: any) =>
    ...
    if (policyBypass.repository_roles) {
      checks.repository_roles = policyBypass.repository_roles.map(
        (role: any) => {
          const roleBypass = bypassActors.find(
            (actor: any) =>
    ...
    if (policyBypass.deploy_keys) {
      const deployKeyBypass = bypassActors.find(
        (actor: any) => actor.actor_type === "DeployKey",
      );
      checks.deploy_keys = {
        ...
        actual_mode: deployKeyBypass
          ...
          : "not configured",
        passed:
          policyBypass.deploy_keys.allow === !!deployKeyBypass &&
          (!deployKeyBypass ||
    ...
    const allBypassPassed = Object.values(checks).every((check: any) => {
      if (Array.isArray(check)) {
        return check.every((item: any) => item.passed);
      }
      return check.passed;
    });
  ...
  private createResult(passed: boolean, checks: any, info: any): CheckResult {
